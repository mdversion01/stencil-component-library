// ============================================================================
// File: stories/toasts/toasts-component.story-helpers.js
// ============================================================================
...
const sanitizeId = value =>
  String(value || 'toast-story')
    .toLowerCase()
    .replace(/[^a-z0-9_-]+/g, '-')
    .replace(/^-+|-+$/g, '') || 'toast-story';

export const makeIds = (baseMap, context) => {
  const scope = sanitizeId(
    context?.id || 'toast-story',
  );
...
const buildPreviewItem = config => {
  ...
  if (
    config.contentHtml &&
    String(config.contentHtml).trim()
  ) {
    item.contentHtml = String(
      config.contentHtml,
    );
...
const applyHostPropertiesAfterUpgrade = (
  ...
  host.previewToasts = previewToasts;
...
export const createToastComponentPreview = (
  args,
  context,
  {
    labelText,
  ...
  /*
   * Storybook Controls must win over static story configuration.
   */
  const config = {
    ...overrides,
    ...args,
  ...
  const previewToasts =
    definitions.map(toastOverrides =>
      buildPreviewItem({
        ...config,
        ...toastOverrides,
      }),
...
export const buildComponentUsageSource = (
  ...
  /*
   * Current Storybook args must also win in Show Code.
   */
  const config = {
    ...overrides,
    ...args,
...
export const buildMultipleComponentUsageSource = (
  ...
  examples
    .map((example, index) => {
      ...
      /*
       * Priority:
       * overrides < current Controls < individual stacked example
       */
      const combinedArgs = {
        ...args,
        ...exampleArgs,
