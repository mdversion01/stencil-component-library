...
export class DateRangePickerComponent {
  ...
  @Prop() inputId: string = 'datepicker';
  ...
  // -------------------- lifecycle -------------------------
  componentWillLoad() {
    this.placeholderText = this.placeholder ?? `${this.dateFormat} ${this.joinBy} ${this.dateFormat}`;
  ...
  /** Used by Option B (output formatting for start/end) */
  ...
  /** Pick the output format for start/end labels based on flags */
  ...
  private parseDate(dateStr: string): Date | null {
    ...
    if (this.dateFormat === 'YYYY-MM-DD' && parts.length === 3) {
      const y = parseInt(parts[0], 10);
      const m = parseInt(parts[1], 10) - 1;
      const d = parseInt(parts[2], 10);
      return new Date(Date.UTC(y, m, d));
    }
    if (this.dateFormat === 'MM-DD-YYYY' && parts.length === 3) {
  ...
  private validateInput = (val: string) => {
    const pattern = /^(?:\d{4}-\d{2}-\d{2}|\d{2}-\d{2}-\d{4})\s*(?:to|-)\s*(?:\d{4}-\d{2}-\d{2}|\d{2}-\d{2}-\d{4})$/;
  ...
  private handleInputChange = (e: Event) => {
    ...
    const parts = inputValue.split(this.joinBy);
    if (parts.length === 2) {
      ...
      if (sDate && eDate && sDate <= eDate) {
        ...
        this.value = `${this.formatDate(this.startDate, this.dateFormat)} ${this.joinBy} ${this.formatDate(this.endDate, this.dateFormat)}`;
        this.el.setAttribute('value', this.value);
        this.updateSelectedRange();
        this.updateDisplayedDateRange();
        this.validationMessage = '';
        this.dateRangeUpdated.emit({
          startDate: this.formatDate(this.startDate, this.dateFormat),
          endDate: this.formatDate(this.endDate, this.dateFormat),
        });
      } else {
        this.validation = true;
  ...
  private _handleOkClick = () => {
    const s = this.startDate ? this.formatDate(this.startDate, this.dateFormat) : null;
    const e = this.endDate ? this.formatDate(this.endDate, this.dateFormat) : null;
    ...
    if (s && e) {
      ...
      this.dateRangeUpdated.emit({ startDate: s, endDate: e });
  ...
  private handleKeyDown = async (event: KeyboardEvent) => {
    ...
    if (event.key.startsWith('Arrow')) {
      ...
      // 👇 NEW: if the activeElement was replaced by a re-render, use the visual ring as the source of truth
      if (index === -1) {
  ...
  private renderDateRangePicker() {
    ...
    return (
      <div class="date-picker">
        <div class={`range-picker-wrapper${this.plumage ? ' plumage' : ''}`} role="region" aria-label={this.ariaLabel || 'Date Range Picker'}>
          <div class="range-picker-nav mb-1" aria-label="Navigation Controls">
            <button onClick={() => this.prevMonth()} class="range-picker-nav-btn btn-outline-secondary" aria-label="Previous Month">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512">
                <path d="M41.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l160 160c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L109.3 256 246.6 118.6c-12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-160 160z" />
              </svg>
            </button>
            ...
            <div class="selectors">
              ...
              <button onClick={() => this.el.dispatchEvent(new CustomEvent('reset-picker', { bubbles: true, composed: true }))} class="reset-btn" aria-label="Reset Calendar">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                  <path d="M48.5 224L40 224c-13.3 0-24-10.7-24-24L16 72c0-9.7 5.8-18.5 14.8-22.2s19.3-1.7 26.2 5.2L98.6 96.6c87.6-86.5 228.7-86.2 315.8 1c87.5 87.5 87.5 229.3 0 316.8s-229.3 87.5-316.8 0c-12.5-12.5-12.5-32.8 0-45.3s-32.8-12.5 45.3 0c62.5 62.5 163.8 62.5 226.3 0s62.5-163.8 0-226.3c-62.2-62.2-162.7-62.5-225.3-1L185 183c6.9 6.9 8.9 17.2 5.2 26.2s-12.5 14.8-22.2 14.8L48.5 224z"></path>
                </svg>
              </button>
            </div>
  ...
  private renderInputGroupPlumage() {
    const groupClass = ['input-group', 'nowrap', this.size === 'sm' ? 'input-group-sm' : '', this.size === 'lg' ? 'input-group-lg' : '', this.disabled ? 'disabled' : '']
      .filter(Boolean)
      .join(' ');

    return (
      <div class={`plumage${this.formLayout ? ` ${this.formLayout}` : ''}`}>
        <div class={`form-group form-input-group${this.formLayout === 'horizontal' ? ' row' : this.formLayout === 'inline' ? ' row inline' : ''}`}>
          <label
            class={['form-control-label', this.required ? 'required' : '', this.labelHidden ? 'sr-only' : '', this.validation ? 'invalid' : ''].filter(Boolean).join(' ')}
            htmlFor={this.inputId}
            aria-hidden="true"
          >
            {this.formLayout === 'horizontal' || this.formLayout === 'inline' ? `${this.label}:` : this.label}
          </label>
  ...
  private renderInputGroupClassic() {
    const wrapperClass = ['form-group', 'form-input-group-basic', this.formLayout, this.formLayout === 'horizontal' || this.formLayout === 'inline' ? 'row' : '']
      .filter(Boolean)
      .join(' ');
    ...
    const groupClass = ['input-group', this.size === 'sm' ? 'input-group-sm' : '', this.size === 'lg' ? 'input-group-lg' : '', this.validation ? ' is-invalid' : '']
      .filter(Boolean)
      .join(' ');
    ...
    return (
      <div class={this.formLayout || undefined}>
        <div class={wrapperClass}>
          <label id={`${this.inputId}-label`} class={labelClasses} htmlFor={this.inputId} aria-hidden="true">
            {this.formLayout === 'horizontal' || this.formLayout === 'inline' ? `${this.label}:` : this.label}
          </label>
