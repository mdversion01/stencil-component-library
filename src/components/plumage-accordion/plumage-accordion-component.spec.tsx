...
import { PlumageAccordionComponent } from './plumage-accordion-component';
import { Button as ButtonComponent } from '../button/button-component';

describe('plumage-accordion-component', () => {
  ...
  it('renders closed by default (collapse class only; aria-hidden + inert)', async () => {
    const page = await newSpecPage({
      components: [PlumageAccordionComponent, ButtonComponent],
      html: `<plumage-accordion-component target-id="section1"></plumage-accordion-component>`,
    });
  ...
  it('renders open by default when is-open is set (collapse show; aria-hidden removed)', async () => {
    const page = await newSpecPage({
      components: [PlumageAccordionComponent, ButtonComponent],
      html: `<plumage-accordion-component target-id="section2" is-open="true"></plumage-accordion-component>`,
    });
  ...
  it('toggles open/closed on click and uses collapsing + transitionend finalization', async () => {
    const page = await newSpecPage({
      components: [PlumageAccordionComponent, ButtonComponent],
      html: `<plumage-accordion-component target-id="section3"></plumage-accordion-component>`,
    });
  ...
  it('renders icon and switches when toggled', async () => {
    const page = await newSpecPage({
      components: [PlumageAccordionComponent, ButtonComponent],
      html: `<plumage-accordion-component
               accordion
  ...
  it('toggles open/closed with keyboard (Enter and Space) via button-component', async () => {
    const page = await newSpecPage({
      components: [PlumageAccordionComponent, ButtonComponent],
      html: `<plumage-accordion-component target-id="section5"></plumage-accordion-component>`,
    });
  ...
  it('renders and toggles in link mode (anchor-like button)', async () => {
    const page = await newSpecPage({
      components: [PlumageAccordionComponent, ButtonComponent],
      html: `<plumage-accordion-component link target-id="link-section" variant="primary">
  ...
  it('emits toggleEvent(boolean) with the new state', async () => {
    const page = await newSpecPage({
      components: [PlumageAccordionComponent, ButtonComponent],
      html: `<plumage-accordion-component target-id="emit-section"></plumage-accordion-component>`,
    });
  ...
  it('@Watch("isOpen"): updating host prop updates internalOpen + aria-expanded + collapse classes', async () => {
    const page = await newSpecPage({
      components: [PlumageAccordionComponent, ButtonComponent],
      html: `<plumage-accordion-component target-id="watched"></plumage-accordion-component>`,
    });
  ...
  it('dedupes targetId when it collides in DOM (unique IDs used in aria-controls and region id)', async () => {
    ...
    const page = await newSpecPage({
      ...
      template: () => (
        <div>
          <div id="dup"></div>
          <plumage-accordion-component targetId="dup"></plumage-accordion-component>
        </div>
      ),
    });
    await page.waitForChanges();
    ...
    const comp =
      (page.root && page.root.tagName?.toLowerCase() === 'plumage-accordion-component'
        ? (page.root as HTMLElement)
        : (page.root.querySelector('plumage-accordion-component') as HTMLElement | null)) ||
      (page.doc.querySelector('plumage-accordion-component') as HTMLElement | null);
