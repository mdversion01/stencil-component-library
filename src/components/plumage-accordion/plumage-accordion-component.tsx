 ...
 */
@Component({
  tag: 'plumage-accordion-component',
...
export class PlumageAccordionComponent {
  ...
  private resolveIds() {
    ...
    if (existing && existing !== this.hostEl) {
      PlumageAccordionComponent._seq += 1;
      const suffix = `${PlumageAccordionComponent._seq}-${Math.random().toString(36).slice(2, 6)}`;
      const unique = `${resolved}-${suffix}`;
      // eslint-disable-next-line no-console
      console.warn(`[plumage-accordion-component] targetId "${resolved}" already exists in DOM. Using "${unique}" to keep IDs unique.`);
      resolved = unique;
    }
