// ============================================================================
// File: src/components/timepicker/timepicker-component.tsx
// ============================================================================
...
const isNormalizeErr = (result: NormalizeResult): result is NormalizeErr => result.ok === false;
...
@Component({
  ...
  styleUrls: ['./timepicker-styles.scss', '../button/button.scss', '../input-group/input-group-styles.scss', '../input-field/input-field-styles.scss'],
...
export class TimePickerComponent {
  ...
  timeChange!: EventEmitter<TimeChangeDetail>;

  @Event({
    eventName: 'timeInput',
  })
  timeInput!: EventEmitter<TimeInputDetail>;

  @Prop()
  ariaLabel: string = 'Time Picker';
  ...
  @Prop()
  labelText: string = 'Enter Time';

  @Prop()
  inputId: string = 'time-input';

  @Prop()
  inputName: string = 'time';
  ...
  isTwentyFourHourFormat: boolean = true;
  ...
  /**
   * Built-in time format/range validation is always enabled.
   * Kept as a prop for backwards compatibility.
   */
  @Prop()
  timeValidation: boolean = true;

  @Prop()
  twentyFourHourOnly: boolean = false;

  @Prop()
  twelveHourOnly: boolean = false;

  @Prop()
  hideTimepickerBtn: boolean = false;
  ...
  /**
   * Throttle window for timeInput events in milliseconds.
   * Set to 0 to disable throttling.
   */
  @Prop()
  timeInputThrottleMs: number = 50;

  @State()
  private _open: boolean = false;

  @State()
  private _activePart: TimePart = 'hour';

  @State()
  private _parts: TimeParts = {
    hour: 0,
    minute: 0,
    second: 0,
    ampm: 'AM',
  ...
  @State()
  private _warningShown: boolean = false;

  @State()
  private _timeValidationMessage: string = '';

  @State()
  private _userValidationActive: boolean = false;
  ...
  /**
   * Tracks whether a time has actually been supplied or selected.
   *
   * This is intentionally independent from the time value because default-looking
   * values such as 00:00:00 and 12:00 AM are also legitimate user selections.
   */
  private _hasTimeBeenSet: boolean = false;

  /**
   * Tracks whether the text input has actually been edited before a commit.
   * Focusing and blurring an untouched default value must not count as setting
   * a time.
   */
  private _inputWasEdited: boolean = false;

  /**
   * Prevents internal value synchronization from being interpreted as an
   * external consumer-provided value.
   */
  private _isSyncingValueProp: boolean = false;
  ...
  componentWillLoad() {
    ...
    if (initial) {
      this._withEmitSuppressed(() => {
        ...
        this._hasTimeBeenSet = true;

        this._inputWasEdited = false;
      });
    } else {
      this._resetToDefault();
    }

    this._syncUserValidationFromTimeSelection();
  }
  ...
  @Watch('disableTimepicker')
  onDisableTimepickerChange(next: boolean, prev: boolean) {
    if (next === prev) {
      return;
    }
    ...
    if (!next && !this._isPartEnabled(this._activePart)) {
  ...
  onValueChange(next: string, prev: string) {
    ...
    this._withEmitSuppressed(() => {
      ...
      this._syncInvalidClasses();
    });
  }

  @Watch('validation')
  onValidationChange(next: boolean) {
    if (!next) {
  ...
  onValidationMessageChange(next: string) {
    if (!(next ?? '').trim()) {
      this._userValidationActive = false;
  ...
  onFormatChange(next: boolean, prev: boolean) {
    ...
    this._withEmitSuppressed(() => {
      ...
      if (!this._isInputFocused) {
        this._setInputValue(this._partsToInput(this._parts), {
          writeToDom: true,
        });
      }
  ...
  onHideSecondsChange(next: boolean, prev: boolean) {
    ...
    this._withEmitSuppressed(() => {
      if (next && this._activePart === 'second') {
        this._activePart = 'minute';
      }
  ...
  handleDocumentKeydown(ev: KeyboardEvent) {
    if (!this._open || this._isDisabled) {
      return;
    }
  ...
  handleOutsideMouseDown(ev: MouseEvent) {
    if (!this._open) {
      return;
    }
  ...
  private get _hasTimeValidationMessage(): boolean {
    return !!(this._timeValidationMessage ?? '').trim();
  }

  private get _hasUserValidationMessage(): boolean {
    return !!(this.validation ?? false) && !!(this.validationMessage ?? '').trim() && this._userValidationActive;
  }

  private get _isInvalidNow(): boolean {
    return this._hasTimeValidationMessage || this._warningShown || this._hasUserValidationMessage || !this.isValid;
  }
  ...
  private _emitTimeChange(source: TimeChangeDetail['source']) {
    if (this._emitSuppressionDepth > 0) {
      return;
    }
  ...
  private computeDescribedByAttrs() {
    ...
    const timeValidationId = this._hasTimeValidationMessage ? this.getTimeValidationId() : undefined;

    const userValidationId = this._hasUserValidationMessage ? this.getValidationId() : undefined;

    const warningId = this._warningShown ? this.getWarningId() : undefined;

    return this.mergeIdLists(external, this.mergeIdLists(timeValidationId, this.mergeIdLists(userValidationId, warningId)));
  }

  private _defaultTimeForFormat(): {
    parts: TimeParts;
    inputValue: string;
  } {
    ...
    return this.hideSeconds
      ...
      : {
          ...
          inputValue: '12:00:00 AM',
        };
  }

  private _syncUserValidationFromTimeSelection() {
    const hasValidation = !!this.validation && !!(this.validationMessage ?? '').trim();
    ...
    this._userValidationActive = !this._hasTimeBeenSet;
  }

  private _activateUserValidationIfNeeded() {
    if (this.validation && (this.validationMessage ?? '').trim()) {
      this._userValidationActive = !this._hasTimeBeenSet;
    }
  }

  private _clearUserValidationIfResolved() {
    if (this._hasUserValidationMessage && this.isValid && this._hasTimeBeenSet) {
      this._userValidationActive = false;
    }
  }

  private _setTimePatternValidation() {
    this._timeValidationMessage = this._validationMessageForPattern();

    this._warningShown = false;
  }

  private _setTimeRangeValidation() {
    this._timeValidationMessage = '';

    this._warningShown = true;
  }

  private _clearTimeValidation() {
    this._timeValidationMessage = '';
  ...
  private _resetToDefault() {
    ...
    this._hasTimeBeenSet = false;

    this._inputWasEdited = false;

    this.isValid = true;

    this._clearTimeValidation();

    this._activateUserValidationIfNeeded();
  ...
  private _isValidParts(parts: TimeParts): boolean {
    const min = parts.minute >= 0 && parts.minute <= 59;

    const sec = this.hideSeconds ? true : parts.second >= 0 && parts.second <= 59;

    if (this.isTwentyFourHourFormat || this.twentyFourHourOnly) {
      return parts.hour >= 0 && parts.hour <= 23 && min && sec;
    }

    return parts.hour >= 1 && parts.hour <= 12 && min && sec && (parts.ampm === 'AM' || parts.ampm === 'PM');
  }
  ...
  private _syncValuePropFromState() {
    ...
    this._withEmitSuppressed(() => {
      this._isSyncingValueProp = true;

      try {
        this.value = next;
      } finally {
        this._isSyncingValueProp = false;
      }
    });
  }

  private _formatAllowed(nextIs24: boolean): boolean {
    if (this.twentyFourHourOnly && !nextIs24) {
      return false;
    }

    if (this.twelveHourOnly && nextIs24) {
      return false;
    }
  ...
  private _focusActivePart() {
    const sel =
      this._activePart === 'hour' ? '.hour-display' : this._activePart === 'minute' ? '.minute-display' : this._activePart === 'second' ? '.second-display' : '.ampm-display';
  ...
  private _moveActivePart(dir: -1 | 1) {
    ...
    if (idx < 0) {
      ...
      this._activePart = parts[(idx + dir + parts.length) % parts.length] || 'hour';
    }
  ...
  private _onSpinKeyDown = (part: TimePart, ev: KeyboardEvent) => {
    if (this._isInteractionLocked || !this._open) {
      return;
    }
  ...
  private _applySpinDelta(part: TimePart, delta: number) {
    ...
    if (part === 'ampm') {
      this._parts = {
        ...
        ampm: this._parts.ampm === 'AM' ? 'PM' : 'AM',
      };
    } else {
  ...
  private _isPartEnabled(part: TimePart): boolean {
    ...
    if (part === 'ampm') {
      return !(this.isTwentyFourHourFormat || this.twentyFourHourOnly);
    }
  ...
  private _normalizeAndParse(raw: string): NormalizeResult {
    ...
    const parts: TimeParts = {
      ...
      ampm: (ampm || 'AM') as AmPm,
    };

    if (!this._isValidParts(parts)) {
      return {
        ok: false,
        normalized,
        reason: 'range',
  ...
  private _isTransientEditingState(raw: string): boolean {
    ...
    if (this.isTwentyFourHourFormat) {
      return this.hideSeconds ? /^\d{0,2}(:\d{0,2})?$/.test(typed) : /^\d{0,2}(:\d{0,2}){0,2}$/.test(typed);
    }

    return this.hideSeconds ? /^\d{0,2}(:\d{0,2})?(\s?[AP]M?)?$/.test(typed) : /^\d{0,2}(:\d{0,2}){0,2}(\s?[AP]M?)?$/.test(typed);
  }
  ...
  private _applyExternalValue(raw: string): boolean {
    ...
    if (isNormalizeErr(res)) {
      ...
      if (res.reason === 'pattern') {
        this._setTimePatternValidation();
      } else {
        this._setTimeRangeValidation();
      }
    ...
    this._setInputValue(res.normalized, {
      writeToDom: true,
    });
    ...
    this._clearUserValidationIfResolved();

    return true;
  }

  private _syncInvalidClasses() {
    const isInvalid = this._isInvalidNow;

    const label = this.host.querySelector('label');

    const input = this.host.querySelector<HTMLInputElement>('.time-input');

    const clearButton = this.host.querySelector<HTMLButtonElement>('.clear-button');

    const iconButton = this.host.querySelector<HTMLButtonElement>('.time-icon-btn');
    ...
    if (clearButton) {
      clearButton.classList.toggle('invalid', isInvalid);
    }

    if (iconButton) {
      iconButton.classList.toggle('invalid', isInvalid);
    }
  }
  ...
  private _setDropdownVisibility(show: boolean) {
    if (show && this._isInteractionLocked) {
      return;
    }
    ...
    if (show) {
      this._returnFocusEl = trigger || input || this.host;
    ...
    const toFocus = this._returnFocusEl || input || trigger || this.host;
  ...
  private _handleInputBlur = (ev: FocusEvent) => {
  ...
  private _validateTimeInput = (ev: Event) => {
    ...
    const isTransient = this._isTransientEditingState(raw);

    this._setInputValue(raw, {
      writeToDom: false,
    });
    ...
    return (
      <div class="time-picker-container">
        <div class={`time-picker ${!this.twentyFourHourOnly && !this.twelveHourOnly ? 'ms-1' : ''}`}>
          ...
          <div class="timepicker-group">
            <div class={`input-group${groupSize}`}>
              <input
                ...
                id={this.inputId}
                name={this.inputName}
                class={`time-input form-control${this.readOnly ? ' read-only' : this._isDisabled ? '' : isInvalid ? ' is-invalid' : ''}${
                  this.hideTimepickerBtn ? '' : this.readOnly ? ' rounded-end' : ''
                }`}
              ...
              {readOnly ? null : (
                <button
                  type="button"
                  class={`clear-button${this.readOnly ? '' : this._isDisabled ? '' : this.validation ? ' invalid' : ''}`}
              ...
              {this.hideTimepickerBtn || this.readOnly ? null : (
                <button
                  type="button"
                  class={`time-icon input-group-text time-icon-btn${this.readOnly ? '' : this._isDisabled ? '' : this.validation ? ' invalid' : ''}`}
            ...
            {!readOnly && !this.twentyFourHourOnly && !this.twelveHourOnly ? (
              <button
                ...
                onClick={() => {
                  if (this.twentyFourHourOnly || this.twelveHourOnly) {
                    return;
                  }
          ...
          <div
            id={timeValidationId}
            class={`validation-message ${this.readOnly || this._isDisabled ? 'hidden' : this._hasTimeValidationMessage ? '' : 'hidden'}`}
            role="alert"
            aria-live="polite"
          >
            {this._timeValidationMessage}
          ...
          <div
            id={validationId}
            class={`validation-message ${this.readOnly || this._isDisabled ? 'hidden' : this._hasUserValidationMessage ? '' : 'hidden'}`}
            role="alert"
            aria-live="polite"
          >
          ...
          <div id={warningId} class={`warning-message ${this.readOnly || this._isDisabled ? 'hidden' : this._warningShown ? '' : 'hidden'}`} role="alert" aria-live="polite">
