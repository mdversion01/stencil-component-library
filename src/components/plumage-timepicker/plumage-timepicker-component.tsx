// ============================================================================
// File: src/components/timepicker/plumage-timepicker-component.tsx
// ============================================================================
...
@Component({
  ...
  styleUrls: ['../timepicker/timepicker-styles.scss', '../button/button.scss', '../plumage-input-field/plumage-input-field-styles.scss', '../form-styles.scss'],
...
export class PlumageTimepickerComponent {
  ...
  /**
   * Tracks whether a time was explicitly supplied or selected.
   *
   * This must not be inferred from the time value because 00:00:00,
   * 00:00, 12:00:00 AM, and 12:00 AM are valid selected values.
   */
  private _hasTimeBeenSet: boolean = false;

  /**
   * Tracks whether the user actually edited the text input.
   * Focusing and blurring an untouched default does not set a time.
   */
  private _inputWasEdited: boolean = false;

  /**
   * Prevents internal value synchronization from being interpreted as
   * an external consumer value change.
   */
  ...
  componentWillLoad() {
    ...
    if (initial) {
      this._withEmitSuppressed(() => {
        ...
        /*
         * A valid consumer-supplied value counts as explicitly set,
         * including midnight/default-looking values.
         */
  ...
  onValueChange(next: string, prev: string) {
    ...
    this._withEmitSuppressed(() => {
      ...
      /*
       * External valid values are explicitly set regardless of their value.
       */
      this._hasTimeBeenSet = true;
  ...
  onFormatChange(next: boolean, prev: boolean) {
    ...
    this._withEmitSuppressed(() => {
      ...
      /*
       * Format conversion is presentation-only.
       * _hasTimeBeenSet is deliberately preserved.
       */
  ...
  private _clearUserValidationIfResolved() {
    if (this._hasUserValidationError && this.isValid && this._hasTimeBeenSet) {
      this._userValidationActive = false;
    }
  }
  ...
  private _isPartsRangeValid(parts: TimeParts): boolean {
    ...
    if (this.isTwentyFourHourFormat || this.twentyFourHourOnly) {
      ...
      if (parts.ampm !== 'AM' && parts.ampm !== 'PM') {
        return false;
      }
    }
    ...
    if (!this.hideSeconds && (second < 0 || second > 59)) {
      return false;
    }
  ...
  private _normalizeAndParse(raw: string): NormalizeResult {
    ...
    const match = normalized.match(this._pattern());
    ...
    const second = this.hideSeconds ? this._parts.second : Number.parseInt(match[3], 10);

    const ampm = this.isTwentyFourHourFormat || this.twentyFourHourOnly ? 'AM' : ((this.hideSeconds ? match[3] : match[4]) as AmPm);
  ...
  private _handleFocusAndInteraction = (event: Event) => {
    if ((event as Event).stopPropagation) {
  ...
  onDocClickForUnderline(event: Event) {
    ...
    const path = typeof (event as MouseEvent).composedPath === 'function' ? (event as MouseEvent).composedPath() : [];

    const clickedInput = input ? path.includes(input) : false;

    const clickedIcon = iconBtn ? path.includes(iconBtn) : false;
  ...
  private _getFocusableInDropdown(): HTMLElement[] {
    const dropdown = this.qs<HTMLDivElement>('.time-dropdown');

    if (!dropdown || !this._open) {
      return [];
    }

    const nodes = Array.from(
      dropdown.querySelectorAll<HTMLElement>('[role="spinbutton"][tabindex], button:not([disabled]), [href], input:not([disabled]), [tabindex]:not([tabindex="-1"])'),
    );
  ...
  private _trapTab(ev: KeyboardEvent) {
    ...
    const next = forward ? (index >= 0 ? (index + 1) % focusables.length : 0) : index >= 0 ? (index - 1 + focusables.length) % focusables.length : focusables.length - 1;
  ...
  /**
   * Opening or closing the dropdown must not commit or validate the time.
   */
  ...
  /**
   * Moving focus from the input to another control inside this component
   * must not implicitly commit or validate the current value.
   */
  private _handleInputBlur = (event: FocusEvent) => {
    this._isInputFocused = false;

    this._handleFocusAndInteraction(event);
  ...
  private _moveActivePart(dir: -1 | 1) {
    ...
    const index = parts.indexOf(this._activePart);

    if (index < 0) {
      this._activePart = parts[0] || 'hour';
    } else {
      this._activePart = parts[(index + dir + parts.length) % parts.length];
    }
  ...
  private _getPartEl(part: TimePart): HTMLElement | null {
    const selector = part === 'hour' ? '.hour-display' : part === 'minute' ? '.minute-display' : part === 'second' ? '.second-display' : '.ampm-display';

    return this.qs<HTMLElement>(selector);
  }
  ...
  private _applySpinDelta(part: TimePart, delta: number) {
    ...
    this._setInputValue(formatted, {
      writeToDom: true,
    });

    /*
     * Any spinner operation is an explicit selection, even if the final
     * result is a default-looking time such as 00:00:00.
     */
    this._hasTimeBeenSet = true;
  ...
  /**
   * Format switching changes presentation only.
   * It never marks the current time as selected.
   */
  ...
  private _convertTimeFormatInState() {
    ...
    if (this.isTwentyFourHourFormat) {
      let hour = Math.min(12, Math.max(1, parts.hour));
    ...
    let hour = Math.min(23, Math.max(0, parts.hour));
  ...
  private _commitFromInput() {
    ...
    /*
     * An untouched default must remain unset.
     * A user-edited default-looking value counts as explicitly selected.
     */
  ...
  private _validateTimeInput = (ev: Event) => {
    ...
    /*
     * Input events represent an actual user edit.
     */
    ...
    /*
     * A complete valid typed time counts immediately, including:
     * 00:00:00, 00:00, 12:00:00 AM, and 12:00 AM.
     */
    this._hasTimeBeenSet = true;
  ...
  private _handleEnterKeyOnly = (event: KeyboardEvent) => {
    if (this._isInteractionLocked) {
      return;
    }
  ...
  private _clearTime = () => {
    ...
    /*
     * Clear restores the unset/default state.
     */
    this._hasTimeBeenSet = false;
        ...
        <div class={`time-picker ${!this.twentyFourHourOnly && !this.twelveHourOnly ? 'mr-2' : ''}`}>
          ...
          <div class="timepicker-group">
            <div class="dd-position">
              ...
              <div class={`b-underline${this.disabled || this.readOnly ? ' disabled' : isInvalid ? ' invalid' : ''}`} role="presentation">
                <div
                  class={`b-focus${this.disabled || this.readOnly ? ' disabled' : isInvalid ? ' invalid' : ''}`}
                  role="presentation"
                  aria-hidden="true"
                  style={
                    {
                ...
                <div class="time-spinner-wrapper">
                  <div class="time-spinner">
                    ...
                    <span
                      ...
                      onKeyDown={event => this._onSpinKeyDown('hour', event)}
                    >
                      {hourMeta.text}
                    </span>
                  ...
                  <div class="time-spinner">
                    ...
                    <span
                      ...
                      onKeyDown={event => this._onSpinKeyDown('minute', event)}
                    >
                      {minMeta.text}
                    </span>
                    ...
                    <Fragment>
                      ...
                      <div class="time-spinner">
                        ...
                        <span
                          ...
                          onKeyDown={event => this._onSpinKeyDown('second', event)}
                        >
                          {secMeta.text}
                        </span>
                  ...
                  {this.isTwentyFourHourFormat || this.twentyFourHourOnly ? null : (
                    <div class="time-spinner am-pm-spinner" aria-hidden="false">
                      ...
                      <span
                        ...
                        onKeyDown={event => this._onSpinKeyDown('ampm', event)}
                      >
                        {ampmMeta.text}
                      </span>
