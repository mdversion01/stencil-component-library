// src/components/plumage-accordion-container/plumage-accordion-container.tsx
import { Component, Prop, h, State, Element } from '@stencil/core';

@Component({
  tag: 'plumage-accordion-container',
  styleUrl: '../plumage-accordion/plumage-accordion.scss',
  shadow: false,
})
export class PlumageAccordionContainer {
  ...
  private makeUniqueBaseId(raw: string) {
    ...
    console.warn(`[plumage-accordion-container] parent-id "${normalized}" already exists in DOM. Using "${unique}" to keep ARIA IDs unique.`);
    return unique;
  }
  ...
  componentWillLoad() {
    ...
    if (provided) {
      ...
      PlumageAccordionContainer._seq += 1;
      const rand = Math.random().toString(36).slice(2, 7);
      this._baseId = `acc-${PlumageAccordionContainer._seq}-${rand}`;
    }
    ...
    if (iconExplicitlyEmpty) {
      // eslint-disable-next-line no-console
      console.warn('[plumage-accordion-container] "icon" prop is empty. Falling back to default "fas fa-angle-down".');
    }
    ...
    if (iconList.length > 2) {
      // eslint-disable-next-line no-console
      console.warn('[plumage-accordion-container] "icon" prop has more than 2 values. Only the first two will be used.', iconList);
    }
  }
